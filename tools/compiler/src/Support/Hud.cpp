...
#if defined(ARK_DEBUG) || defined(DEBUG) || defined(_DEBUG)
static constexpr bool kHudDebugEnabled = true;
#else
static constexpr bool kHudDebugEnabled = false;
#endif
...
static bool endsWithLF(std::string_view s) {
...
void Hud::renderBannerLocked(std::string_view toolName, std::string_view version,
    ...
    const std::string sub = std::string("target: ") + std::string(targetTripleHint);
...
bool Hud::isVerbose() const {
    return theme_.verbose;
}
...
void Hud::printErr(std::string_view text) {
    emitLine(Stream::Err, text);
}

void Hud::printOut(std::string_view text) {
    emitLine(Stream::Out, text);
}
...
void Hud::renderBlockLocked(const LogBlock& b) {
    ...
    const size_t w = std::max(contentW, titleW) + 2;
    ...
    for (auto ln : lines) row(ln, false, true);
    bot();
}
...
Step::Step(Hud& hud, StepMeta meta) : hud_(&hud) {
    hud_->stepBegin(meta);
}
