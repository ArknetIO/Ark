...
// -----------------------------------------------------------------------------
// Panic Handling (Portable setjmp/longjmp)
// -----------------------------------------------------------------------------
// POSIX builds use signal-aware jump buffers.
// Windows/MSVC uses the standard C jump buffer APIs.
// -----------------------------------------------------------------------------
#if defined(_WIN32)
static thread_local jmp_buf g_panic_env;
  #define ARK_PANIC_SETJMP(env) setjmp(env)
  #define ARK_PANIC_LONGJMP(env, value) longjmp(env, value)
#else
static thread_local sigjmp_buf g_panic_env;
  #define ARK_PANIC_SETJMP(env) sigsetjmp(env, 1)
  #define ARK_PANIC_LONGJMP(env, value) siglongjmp(env, value)
#endif
...
static void arkcPanicHandler(const char* msg, const char* file, int32_t line, int32_t col) {
    ...
    std::fprintf(stderr, "  Loc:  %s:%d:%d\n", file ? file : "<unknown>", line, col);
    ARK_PANIC_LONGJMP(g_panic_env, 1);
}
...
int JIT::Run(mlir::ModuleOp module, llvm::StringRef runtimeDir) {
    ...
    // If the hook symbol does not exist, consume the error and continue.
    ...
    // 6. Execute Main
    if (ARK_PANIC_SETJMP(g_panic_env) == 0) {
