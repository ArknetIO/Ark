...
std::string Linker::findCompilerExe() {
    hud_.error("DEBUG llvmBinDir=[" + cfg_.llvmBinDir + "]");
    hud_.error("DEBUG clangOverride=[" + cfg_.clangOverride + "]");
    hud_.error("DEBUG mlirTranslateOverride=[" + cfg_.mlirTranslateOverride + "]");

    const std::vector<std::string> roots = configuredToolRoots(cfg_);
    for (const auto& root : roots) {
        hud_.error("DEBUG toolRoot=[" + root + "]");
    }
