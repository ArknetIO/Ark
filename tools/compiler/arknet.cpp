...
static void printVersion(bool asJson) {
...
void registerVersionCmd(CLI::App& app) {
    auto* cmd = app.add_subcommand("version", "Print version information");
    auto jsonFlag = std::make_shared<bool>(false);

    cmd->add_flag("--json", *jsonFlag, "Print version info as JSON");
    cmd->callback([jsonFlag]() {
        printVersion(*jsonFlag);
    });
}

void registerTopLevelVersionFlags(CLI::App& app) {
    app.add_flag_callback("-V,--version", []() {
        printVersion(false);
        throw CLI::Success();
    }, "Print version information and exit");

    app.add_flag_callback("--version-json", []() {
        printVersion(true);
        throw CLI::Success();
    }, "Print version info as JSON and exit");
}

void registerSubcommands(CLI::App& app) {
