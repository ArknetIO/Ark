...
struct Hasher::Impl {
    HashAlgo algo;
    size_t out_len_config = 0;
    std::vector<uint8_t> key_storage;

    std::unique_ptr<crypto_hash_sha256_state> sha256_state;
    std::unique_ptr<crypto_hash_sha512_state> sha512_state;
    std::unique_ptr<crypto_generichash_state> blake2b_state;
    std::unique_ptr<blake3_hasher> blake3_state;
        ...
        : algo(a)
    {
        switch (algo) {
            case HashAlgo::SHA256:
                sha256_state = std::make_unique<crypto_hash_sha256_state>();
                break;
            case HashAlgo::SHA512:
                sha512_state = std::make_unique<crypto_hash_sha512_state>();
                break;
            case HashAlgo::BLAKE2B:
                blake2b_state = std::make_unique<crypto_generichash_state>();
            ...
            case HashAlgo::BLAKE3:
                blake3_state = std::make_unique<blake3_hasher>();
                out_len_config = (out_len == 0) ? 32 : out_len;
    ...
    void reset() {
        switch (algo) {
            case HashAlgo::SHA256:
                crypto_hash_sha256_init(sha256_state.get());
                break;
            case HashAlgo::SHA512:
                crypto_hash_sha512_init(sha512_state.get());
                break;
            case HashAlgo::BLAKE2B:
            ...
            case HashAlgo::BLAKE3:
                if (!key_storage.empty()) {
                    blake3_hasher_init_keyed(blake3_state.get(), key_storage.data());
                } else {
                    blake3_hasher_init(blake3_state.get());
                }
                break;
        }
    }
...
void Hasher::update(std::span<const uint8_t> data) {
    switch (_impl->algo) {
        case HashAlgo::SHA256:
            crypto_hash_sha256_update(_impl->sha256_state.get(), data.data(), data.size());
            break;
        case HashAlgo::SHA512:
            crypto_hash_sha512_update(_impl->sha512_state.get(), data.data(), data.size());
            break;
        case HashAlgo::BLAKE2B:
            crypto_generichash_update(_impl->blake2b_state.get(), data.data(), data.size());
            break;
        case HashAlgo::BLAKE3:
            blake3_hasher_update(_impl->blake3_state.get(), data.data(), data.size());
            break;
    }
}
...
std::vector<uint8_t> Hasher::finalize() {
    ...
    switch (_impl->algo) {
        case HashAlgo::SHA256:
            ...
            crypto_hash_sha256_final(_impl->sha256_state.get(), out.data());
            break;
        case HashAlgo::SHA512:
            out.resize(crypto_hash_sha512_BYTES);
            crypto_hash_sha512_final(_impl->sha512_state.get(), out.data());
            break;
        case HashAlgo::BLAKE2B:
            out.resize(_impl->out_len_config);
            crypto_generichash_final(_impl->blake2b_state.get(), out.data(), out.size());
            break;
        case HashAlgo::BLAKE3:
            out.resize(_impl->out_len_config);
            blake3_hasher_finalize(_impl->blake3_state.get(), out.data(), out.size());
            break;
    }
...
std::vector<uint8_t> Hasher::compute_blake2b(
...
std::vector<uint8_t> Hasher::compute_blake3(
    std::span<const uint8_t> data,
    size_t out_len,
    std::span<const uint8_t> key
) {
